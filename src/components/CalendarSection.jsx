    ...
    <div className="bg-[#0f0f0f] border border-[#1f1f1f] rounded-xl p-6 h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
           ...
           <button className="flex items-center gap-2 text-[10px] text-gray-400 border border-[#2d2d2d] rounded-lg px-3 py-1.5 hover:bg-[#1f1f1f] transition-colors">
      ...
      {/* Calendar Grid */}
      <div className="flex-1 grid grid-cols-7 border border-[#1f1f1f] rounded-lg bg-[#0a0a0a] overflow-hidden min-h-[400px]">
         {weekDays.map((day, index) => {
            ...
            return (
               <div key={dateKey} className={`border-r border-[#1f1f1f] last:border-r-0 flex flex-col ${isToday ? 'bg-[#141414]' : ''}`}>
                  
                  {/* Day Header */}
                  <div className="p-3 border-b border-[#1f1f1f] text-center">
                  ...
                  <div className="p-2 space-y-2 flex-1 overflow-y-auto no-scrollbar">
                     {dayTasks.map(task => (
                        <div key={task.id} className="bg-[#1a1a1a] border border-[#2d2d2d] rounded p-2 hover:border-gray-500 transition-colors cursor-pointer group">
                           {/* Task Type/Icon */}
